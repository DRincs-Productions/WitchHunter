# TODO: Translation updated at 2023-07-25 15:55

# game/scripts/quests/tarani/paizuriTarani.rpy:5
translate french paizuriTarani_54173553:

    # cedrick "И так, класс, сегодня вас ждет новая контрольная работа. Надеюсь все успели подготовиться"
    cedrick "Alors, les enfants, il y a un nouveau test aujourd'hui. J'espère que tout le monde a eu le temps de se préparer"

# game/scripts/quests/tarani/paizuriTarani.rpy:6
translate french paizuriTarani_e210dc2b:

    # haylin "О нееет, только не снова.."
    haylin "Oh non, pas encore..."

# game/scripts/quests/tarani/paizuriTarani.rpy:7
translate french paizuriTarani_dfbdfe87:

    # cedrick "Мы лишь проверяем уровень ваших знаний, Хай Лин. В правильных руках любое знание превращается в силу, не стоит это недооценивать. Верно, Тарани?"
    cedrick "Nous ne faisons que tester votre niveau de connaissance, Hay Lin. Dans les bonnes mains, toute connaissance se transforme en pouvoir, ne la sous-estimez pas. N'est-ce pas, Taranee ?"

# game/scripts/quests/tarani/paizuriTarani.rpy:8
translate french paizuriTarani_1e9e5a76:

    # tarani "..Верно"
    tarani "...Exact."

# game/scripts/quests/tarani/paizuriTarani.rpy:9
translate french paizuriTarani_0f374d4c:

    # haylin "Ну конечно, Тарани то всегда ко всему готова!"
    haylin "Bien sûr, Taranee est toujours prêt à tout !"

# game/scripts/quests/tarani/paizuriTarani.rpy:10
translate french paizuriTarani_c78b3858:

    # cedrick "(Ох, как оказалось, не ко всему)"
    cedrick "(Oh, comme il s'avère, pas à tout)"

# game/scripts/quests/tarani/paizuriTarani.rpy:12
translate french paizuriTarani_d4706847:

    # centered "{size=37}После урока..{/size}"
    centered "{size=37}Après la leçon..{/size}"

# game/scripts/quests/tarani/paizuriTarani.rpy:18
translate french paizuriTarani_36adae20:

    # cedrick "Тарани, не поможешь мне с проверкой контрольных?"
    cedrick "Taranee, pouvez-vous m'aider à vérifier les contrôles ?"

# game/scripts/quests/tarani/paizuriTarani.rpy:19
translate french paizuriTarani_4eda9780:

    # tarani "..Ребят, не ждите меня, я должна помочь Профессору Рику.."
    tarani "..Les gars, ne m'attendez pas, je dois aider le Professeur Rick..."

# game/scripts/quests/tarani/paizuriTarani.rpy:20
translate french paizuriTarani_9b0edb94:

    # irma "Окей, не задерживайся там"
    irma "OK, ne reste pas là trop longtemps"

# game/scripts/quests/tarani/paizuriTarani.rpy:21
translate french paizuriTarani_1ee5b6c4:

    # tarani "Я постараюсь.."
    tarani "Je vais essayer de.."

# game/scripts/quests/tarani/paizuriTarani.rpy:25
translate french paizuriTarani_42fdc3a9:

    # cedrick "Мда, парень ни на что негоден.. Надеюсь хоть ты за него постараешься, Тарани"
    cedrick "Oui, ce type n'est pas bon. J'espère que vous allez au moins essayer pour lui, Taranee."

# game/scripts/quests/tarani/paizuriTarani.rpy:29
translate french paizuriTarani_6431c766:

    # tarani "Хватит давить на меня.."
    tarani "Arrêtez de me pousser..."

# game/scripts/quests/tarani/paizuriTarani.rpy:32
translate french paizuriTarani_b0d660bb:

    # cedrick "Ну, этого бы не произошло, если бы ты вовремя надавила на Найджела и он взялся бы за ум. А теперь разве что ты можешь взяться за мой член"
    cedrick "Ça ne serait pas arrivé si tu avais appuyé sur Nigel à temps et qu'il avait pris son cerveau. Maintenant, à moins que tu puisses avoir une prise sur ma bite."

# game/scripts/quests/tarani/paizuriTarani.rpy:33
translate french paizuriTarani_96bd030f:

    # tarani "......"
    tarani "......"

# game/scripts/quests/tarani/paizuriTarani.rpy:34
translate french paizuriTarani_63b5a7de:

    # cedrick "Хм, между твоими девочками довольно приятно, но не хочешь ускорить процесс? Возьми его в рот."
    cedrick "Hmm, c'est assez agréable entre vos filles, mais voulez-vous accélérer les choses ? Prends-le dans ta bouche."

# game/scripts/quests/tarani/paizuriTarani.rpy:37
translate french paizuriTarani_d55b0eb0:

    # tarani "Ч-что?! Я не стану брать эту штуку себе в рот, ты понял!"
    tarani "Qu-Quoi ? ! Je ne prends pas ce truc dans ma bouche, tu comprends !"

# game/scripts/quests/tarani/paizuriTarani.rpy:40
translate french paizuriTarani_7f2b788d:

    # tarani "(Черт, да я даже Найджелу не отсасывала, а тебе не отсосу уж точно. Я вообще не уверена, что эта штука сможет поместиться мне в рот...)"
    tarani "(Bon sang, je n'ai même pas sucé la bite de Nigel, et je suis sûr que je n'ai pas sucé la tienne. Je ne suis même pas sûr que ce truc puisse rentrer dans ma bouche...)"

# game/scripts/quests/tarani/paizuriTarani.rpy:41
translate french paizuriTarani_c4bfbd29:

    # cedrick "Ох, какая ты недотрога. Ты уже дрочишь мне, прячась от всех под столом, чего уж стыдиться?"
    cedrick "Oh, tu es tellement prude. Tu me branles déjà en te cachant sous la table, il n'y a pas de quoi avoir honte."

# game/scripts/quests/tarani/paizuriTarani.rpy:44
translate french paizuriTarani_8d97f227:

    # tarani "Я.. Думаю вкус этой штуковины будет мне противен"
    tarani "Я.. Je pense que le goût de ce truc va me dégoûter."

# game/scripts/quests/tarani/paizuriTarani.rpy:47
translate french paizuriTarani_a9d63d3f:

    # cedrick "Думаешь? Так ты что, еще никому не отсасывала? Хах, а в тот раз так резво набросилась на мой член, словно все уже с ним успела проделать"
    cedrick "Tu crois ça ? Donc tu n'as encore sucé la bite de personne ? Huh, cette fois tu étais si rapide à sauter sur ma bite, comme si tu avais tout fait avec."

# game/scripts/quests/tarani/paizuriTarani.rpy:48
translate french paizuriTarani_5c034e7a:

    # cedrick "Постой-ка, получается ты не сосала Найджелу? Вы с ним что, еще даже не спали?"
    cedrick "Attends une minute, tu n'as pas sucé Nigel ? Tu n'as même pas encore couché avec lui ?"

# game/scripts/quests/tarani/paizuriTarani.rpy:51
translate french paizuriTarani_02f8aba2:

    # tarani "...Заткнись"
    tarani "...Tais-toi"

# game/scripts/quests/tarani/paizuriTarani.rpy:54
translate french paizuriTarani_867b5a86:

    # cedrick "Ох, бедный пацан. Как же он опечалится, когда я заберу твой первый раз"
    cedrick "Oh, pauvre enfant. Comme il sera triste quand je prendrai ton premier..."

# game/scripts/quests/tarani/paizuriTarani.rpy:55
translate french paizuriTarani_253dfb5e:

    # tarani "Этого никогда не произойдет, ты, придурок.."
    tarani "Ça n'arrivera jamais, espèce d'idiot..."

# game/scripts/quests/tarani/paizuriTarani.rpy:56
translate french paizuriTarani_e650d6ae:

    # cedrick "Ну, прямо сейчас ты дрочишь мне, а не ему. У меня как минимум есть шансы, не считаешь?"
    cedrick "Pour l'instant, c'est toi qui me branles, pas lui. J'ai au moins une chance, tu ne crois pas ?"

# game/scripts/quests/tarani/paizuriTarani.rpy:59
translate french paizuriTarani_ae2ceb25:

    # tarani "Кончай уже.."
    tarani "Arrêtez déjà..."

# game/scripts/quests/tarani/paizuriTarani.rpy:60
translate french paizuriTarani_e44c2b10:

    # cedrick "Ох, ты ускорилась? Да, с этой злостью в глазах ты мне нравишься больше. Дрянная девчонка, продолжай!"
    cedrick "Oh, tu as accéléré ? Ouais, je te préfère avec cette colère dans tes yeux. Méchante fille, vas-y !"

# game/scripts/quests/tarani/paizuriTarani.rpy:61
translate french paizuriTarani_8a78f40a:

    # cedrick "Я уже близко.. Почти.."
    cedrick "Je suis proche... Presque..."

# game/scripts/quests/tarani/paizuriTarani.rpy:66
translate french paizuriTarani_a20cefa7:

    # "..."
    "..."

# game/scripts/quests/tarani/paizuriTarani.rpy:69
translate french paizuriTarani_92351330:

    # tarani "Ублюдок, ты все лицо мне забрызгал. Мерзость.."
    tarani "Enfoiré, tu m'as éclaboussé le visage. Dégoûtant..."

# game/scripts/quests/tarani/paizuriTarani.rpy:70
translate french paizuriTarani_8c6480e9:

    # cedrick "Привыкай, дорогая, это не последний раз, когда твое лицо заливается моей спермой"
    cedrick "Habitue-toi, chérie, ce ne sera pas la dernière fois que ton visage sera trempé dans mon sperme."

# game/scripts/quests/tarani/paizuriTarani.rpy:74
translate french paizuriTarani_a1eb2b24:

    # tarani "(Это мы еще посмотрим, засранец.. Я не могу позволить, чтобы это продолжалось.. Пока он не забрал мою девственность..)"
    tarani "(On verra ça, connard. Je ne peux pas laisser faire ça... Jusqu'à ce qu'il prenne ma virginité...)"

