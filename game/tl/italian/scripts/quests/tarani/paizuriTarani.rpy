# TODO: Translation updated at 2023-07-25 15:55

# game/scripts/quests/tarani/paizuriTarani.rpy:5
translate italian paizuriTarani_54173553:

    # cedrick "И так, класс, сегодня вас ждет новая контрольная работа. Надеюсь все успели подготовиться"
    cedrick "Allora, classe, oggi c'è un nuovo quiz. Spero che tutti abbiano avuto il tempo di prepararsi"

# game/scripts/quests/tarani/paizuriTarani.rpy:6
translate italian paizuriTarani_e210dc2b:

    # haylin "О нееет, только не снова.."
    haylin "Oh no, non di nuovo..."

# game/scripts/quests/tarani/paizuriTarani.rpy:7
translate italian paizuriTarani_dfbdfe87:

    # cedrick "Мы лишь проверяем уровень ваших знаний, Хай Лин. В правильных руках любое знание превращается в силу, не стоит это недооценивать. Верно, Тарани?"
    cedrick "Stiamo solo testando il tuo livello di conoscenza, Hay Lin. Nelle mani giuste ogni conoscenza si trasforma in potere, non sottovalutatelo. Giusto, Taranee?"

# game/scripts/quests/tarani/paizuriTarani.rpy:8
translate italian paizuriTarani_1e9e5a76:

    # tarani "..Верно"
    tarani "...Giusto."

# game/scripts/quests/tarani/paizuriTarani.rpy:9
translate italian paizuriTarani_0f374d4c:

    # haylin "Ну конечно, Тарани то всегда ко всему готова!"
    haylin "Naturalmente, Taranee è sempre pronta a tutto!"

# game/scripts/quests/tarani/paizuriTarani.rpy:10
translate italian paizuriTarani_c78b3858:

    # cedrick "(Ох, как оказалось, не ко всему)"
    cedrick "(Oh, come si è scoperto, non a tutto)"

# game/scripts/quests/tarani/paizuriTarani.rpy:12
translate italian paizuriTarani_d4706847:

    # centered "{size=37}После урока..{/size}"
    centered "{size=37}Dopo la lezione..{/size}"

# game/scripts/quests/tarani/paizuriTarani.rpy:18
translate italian paizuriTarani_36adae20:

    # cedrick "Тарани, не поможешь мне с проверкой контрольных?"
    cedrick "Taranee, puoi aiutarmi a controllare i controlli?"

# game/scripts/quests/tarani/paizuriTarani.rpy:19
translate italian paizuriTarani_4eda9780:

    # tarani "..Ребят, не ждите меня, я должна помочь Профессору Рику.."
    tarani "Ragazzi, non aspettate, devo aiutare il professor Rick..."

# game/scripts/quests/tarani/paizuriTarani.rpy:20
translate italian paizuriTarani_9b0edb94:

    # irma "Окей, не задерживайся там"
    irma "OK, non rimanete lì troppo a lungo"

# game/scripts/quests/tarani/paizuriTarani.rpy:21
translate italian paizuriTarani_1ee5b6c4:

    # tarani "Я постараюсь.."
    tarani "Cercherò di..."

# game/scripts/quests/tarani/paizuriTarani.rpy:25
translate italian paizuriTarani_42fdc3a9:

    # cedrick "Мда, парень ни на что негоден.. Надеюсь хоть ты за него постараешься, Тарани"
    cedrick "Sì, il ragazzo non è bravo. Spero che almeno ci proverai, Taranee."

# game/scripts/quests/tarani/paizuriTarani.rpy:29
translate italian paizuriTarani_6431c766:

    # tarani "Хватит давить на меня.."
    tarani "Smettetela di spingermi..."

# game/scripts/quests/tarani/paizuriTarani.rpy:32
translate italian paizuriTarani_b0d660bb:

    # cedrick "Ну, этого бы не произошло, если бы ты вовремя надавила на Найджела и он взялся бы за ум. А теперь разве что ты можешь взяться за мой член"
    cedrick "Beh, questo non sarebbe successo se aveste premuto Nigel in tempo e se lui avesse fatto un po' di chiarezza. Ora, a meno che tu non riesca a prendere in mano il mio cazzo."

# game/scripts/quests/tarani/paizuriTarani.rpy:33
translate italian paizuriTarani_96bd030f:

    # tarani "......"
    tarani "......"

# game/scripts/quests/tarani/paizuriTarani.rpy:34
translate italian paizuriTarani_63b5a7de:

    # cedrick "Хм, между твоими девочками довольно приятно, но не хочешь ускорить процесс? Возьми его в рот."
    cedrick "Hmm, è molto bello tra le tue ragazze, ma vuoi accelerare le cose? Prendetelo in bocca."

# game/scripts/quests/tarani/paizuriTarani.rpy:37
translate italian paizuriTarani_d55b0eb0:

    # tarani "Ч-что?! Я не стану брать эту штуку себе в рот, ты понял!"
    tarani "Cosa? Non prenderò quella cosa in bocca, capisci?"

# game/scripts/quests/tarani/paizuriTarani.rpy:40
translate italian paizuriTarani_7f2b788d:

    # tarani "(Черт, да я даже Найджелу не отсасывала, а тебе не отсосу уж точно. Я вообще не уверена, что эта штука сможет поместиться мне в рот...)"
    tarani "(Diavolo, non ho nemmeno succhiato il cazzo di Nigel, e di sicuro non ho succhiato il tuo. Non sono nemmeno sicuro che quella cosa possa entrare nella mia bocca...)."

# game/scripts/quests/tarani/paizuriTarani.rpy:41
translate italian paizuriTarani_c4bfbd29:

    # cedrick "Ох, какая ты недотрога. Ты уже дрочишь мне, прячась от всех под столом, чего уж стыдиться?"
    cedrick "Oh, sei proprio una puritana. Mi stai già facendo una sega nascondendoti sotto il tavolo, cosa c'è da vergognarsi?"

# game/scripts/quests/tarani/paizuriTarani.rpy:44
translate italian paizuriTarani_8d97f227:

    # tarani "Я.. Думаю вкус этой штуковины будет мне противен"
    tarani "Я.. Credo che il sapore di questa roba mi disgusti."

# game/scripts/quests/tarani/paizuriTarani.rpy:47
translate italian paizuriTarani_a9d63d3f:

    # cedrick "Думаешь? Так ты что, еще никому не отсасывала? Хах, а в тот раз так резво набросилась на мой член, словно все уже с ним успела проделать"
    cedrick "Lo pensi anche tu? Quindi non hai ancora succhiato il cazzo a nessuno? Quella volta che sei stata così veloce a saltare sul mio cazzo, come se avessi fatto tutto con esso."

# game/scripts/quests/tarani/paizuriTarani.rpy:48
translate italian paizuriTarani_5c034e7a:

    # cedrick "Постой-ка, получается ты не сосала Найджелу? Вы с ним что, еще даже не спали?"
    cedrick "Aspetta un attimo, quindi non hai succhiato Nigel? Non sei ancora andata a letto con lui?"

# game/scripts/quests/tarani/paizuriTarani.rpy:51
translate italian paizuriTarani_02f8aba2:

    # tarani "...Заткнись"
    tarani "...Zitto"

# game/scripts/quests/tarani/paizuriTarani.rpy:54
translate italian paizuriTarani_867b5a86:

    # cedrick "Ох, бедный пацан. Как же он опечалится, когда я заберу твой первый раз"
    cedrick "Oh, povero ragazzo. Quanto sarà triste quando prenderò la tua prima"

# game/scripts/quests/tarani/paizuriTarani.rpy:55
translate italian paizuriTarani_253dfb5e:

    # tarani "Этого никогда не произойдет, ты, придурок.."
    tarani "Non succederà mai, idiota..."

# game/scripts/quests/tarani/paizuriTarani.rpy:56
translate italian paizuriTarani_e650d6ae:

    # cedrick "Ну, прямо сейчас ты дрочишь мне, а не ему. У меня как минимум есть шансы, не считаешь?"
    cedrick "Beh, in questo momento sei tu a fare una sega a me, non lui. Ho almeno una possibilità, non credi?"

# game/scripts/quests/tarani/paizuriTarani.rpy:59
translate italian paizuriTarani_ae2ceb25:

    # tarani "Кончай уже.."
    tarani "Smettila..."

# game/scripts/quests/tarani/paizuriTarani.rpy:60
translate italian paizuriTarani_e44c2b10:

    # cedrick "Ох, ты ускорилась? Да, с этой злостью в глазах ты мне нравишься больше. Дрянная девчонка, продолжай!"
    cedrick "Oh, hai accelerato? Sì, mi piaci di più con quella rabbia negli occhi. Ragazza cattiva, vai avanti!"

# game/scripts/quests/tarani/paizuriTarani.rpy:61
translate italian paizuriTarani_8a78f40a:

    # cedrick "Я уже близко.. Почти.."
    cedrick "Sono vicino... Quasi..."

# game/scripts/quests/tarani/paizuriTarani.rpy:66
translate italian paizuriTarani_a20cefa7:

    # "..."
    "..."

# game/scripts/quests/tarani/paizuriTarani.rpy:69
translate italian paizuriTarani_92351330:

    # tarani "Ублюдок, ты все лицо мне забрызгал. Мерзость.."
    tarani "Bastardo, mi hai schizzato in faccia. Sei disgustoso..."

# game/scripts/quests/tarani/paizuriTarani.rpy:70
translate italian paizuriTarani_8c6480e9:

    # cedrick "Привыкай, дорогая, это не последний раз, когда твое лицо заливается моей спермой"
    cedrick "Abituati, tesoro, non sarà l'ultima volta che la tua faccia verrà inzuppata dal mio sperma."

# game/scripts/quests/tarani/paizuriTarani.rpy:74
translate italian paizuriTarani_a1eb2b24:

    # tarani "(Это мы еще посмотрим, засранец.. Я не могу позволить, чтобы это продолжалось.. Пока он не забрал мою девственность..)"
    tarani "(Questo lo vedremo, stronzo. Non posso permettere che questo vada avanti... Fino a quando non prenderà la mia verginità...)"

